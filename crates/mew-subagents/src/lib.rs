...
pub enum SubagentEvent {
    ...
    PermissionRequest {
        ...
        tx: oneshot::Sender<mew_hooks::PermissionDecision>,
    },
}
...
fn load_agent_file(path: &Path) -> Result<SubagentDef, SubagentError> {
    ...
    let (name, description, model, model_small, model_big, tools, max_turns, body) = match parsed {
        Some((fm, body)) => {
...
mod tests {
    ...
    fn test_config_override() {
        let tmp = tempfile::tempdir().unwrap();
        let cwd = tmp.path();
        write_agent_full(cwd, "explore", "desc", None, None, None, None, None, "body");
