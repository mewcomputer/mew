...
async fn build_daemon_server(
    ...
    // Resolve model/provider from flags or state.
    let state = mew_config::load_state().unwrap_or_default();
    let provider = resolve_provider(Some(provider_flag.to_string()), &state);
    let model = resolve_model_opt(model_flag, &state);
...
///
/// Reuses `run_daemon`'s server-building logic by calling it with no socket/port
/// (which would be a no-op) and then taking the server components for iroh.
/// In practice, we build the server inline to avoid starting a listener.
#[cfg(feature = "iroh")]
async fn run_daemon_iroh(
    ...
    let allowlist_path = mew_daemon::default_allowlist_path();
    let secret_key_path = mew_daemon::default_secret_key_path();
    info!(allowlist = %allowlist_path.display(), "starting iroh listener");

    let secret_key = mew_daemon::load_or_create_secret_key(&secret_key_path)?;
...
async fn pair_cmd() -> Result<()> {
    ...
    let endpoint = Endpoint::builder(iroh::endpoint::presets::N0)
        ...
        .alpns(vec![mew_daemon::MEW_ALPN.to_vec()])
        .bind()
        .await
    ...
    let allowlist_path = mew_daemon::default_allowlist_path();
    let allowlist = Arc::new(mew_daemon::NodeIdAllowlist::load(allowlist_path.clone())?);
