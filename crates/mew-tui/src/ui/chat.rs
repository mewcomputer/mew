...
fn render_single_tool_call(
    ...
    if let Some(ToolDisplayState::Completed { output, diff }) = app.tool_states.get(&tc.base.id) {
        if !output.is_empty() {
...
mod tests {
    ...
    fn test_wrap_tool_line_continuation_rows_have_indent() {
        ...
        // start with the indent so wrapped text aligns with the first row's
