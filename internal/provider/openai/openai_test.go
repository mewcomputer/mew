...
func TestAdapterTextOnly(t *testing.T) {
	...
	a := New("test", srv.URL, "gpt-4", "fake-key")
	ctx := t.Context()
	...
	// Expect: part_start(text), part_delta, ..., part_end, message_end(stop)
	_, ok := parts[0].(provider.EventPartStart)
	if !ok {
		t.Fatalf("expected EventPartStart, got %T", parts[0])
	}
...
func TestAdapterToolCall(t *testing.T) {
	...
	// Verify that a ToolCallPart was emitted with correct name and parsed arguments.
	var found bool
		...
		if ps, ok := ev.(provider.EventPartStart); ok {
			if tc, ok := ps.Part.(*message.ToolCallPart); ok {
				...
				if tc.CallID != "call_abc" {
					t.Fatalf("expected call_id call_abc, got %q", tc.CallID)
				}
