...
| `settings` | Open the in-app settings overlay with the default config |
| `settings_config <path>` | Open the settings overlay loaded from a TOML config file |

### Example: settings overlay capture

Use `settings_config` with a controlled TOML file so the capture is deterministic
regardless of the user's real `config.toml`:

```
# Load a minimal config and open the settings overlay
size 100 30
settings_config "config-only-zai.toml"
snapshot settings-open
screenshot settings-open.png

# Navigate to the z-ai account and view its details
key down
key down
key tab
snapshot account-details
screenshot account-details.png
key esc
```
