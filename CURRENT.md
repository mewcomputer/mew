...
Added the full oh-my-pi/hashline edit flow to mew through a new `mew-hashline`
crate. Existing `EditStrReplace` and `Write` tools remain unchanged.

### What was done

- Created `crates/mew-hashline/` and registered it in the workspace.
- Stable 4-hex xxHash32 hashing with LF/BOM normalization.
- Line tokenizer/parser for `SWAP`, `DEL`, `INS.*`, `SWAP.BLK`, `DEL.BLK`,
  `INS.BLK.POST`, `REM`, and `MV`.
- In-memory `SnapshotStore` with per-path history and seen-line tracking.
- `apply_edits` engine plus tree-sitter-backed block resolution for Rust,
  TypeScript, Python, Go, and Markdown.
- Simple stale-tag recovery (exact-match + anchor-content replay).
- `Patcher` orchestrator with preflight/all-or-nothing semantics.
- Updated `mew-tools/src/tools/read.rs` to emit `[path#hash]` headers and
  numbered lines, recording snapshots for hashline edits.
- Rewrote `mew-tools/src/tools/edit_hashline.rs` to delegate to
  `mew-hashline::Patcher`.
- Added `ProviderConfig.disable_hashline` so weak models can fall back to
  `EditStrReplace`.
- Threaded a shared `snapshot_store` through `mew-agent` into `ToolCtxShared`.
- Fixed the `mew-daemon/tests/concurrency.rs` test that was failing because
  session-title generation consumes one provider script.
- Fixed an unrelated stale test in `mew-prompts/src/subagent.rs` that expected
  the old reviewer prompt to contain `warning`; now checks `high` to match the
  current severity enum.

### Documentation

- Added user guide at `site/src/content/docs/docs/hashline.md`.
- Added developer/architecture doc at `site/src/content/docs/docs/dev-hashline.md`.
- Updated `site/src/content/docs/docs/tools.md`, `configuration.md`, and
  `dev-architecture.md` to cover hashline and `disable_hashline`.
- Updated `site/src/content/docs/docs/dev-tools.md` to mention the shared
  `snapshot_store` in `ToolCtxShared`.
- Registered the new pages in `site/astro.config.mjs` and verified the site
  builds cleanly.
...
- `cd site && pnpm run build` (site build) clean

### Next steps

- Replacement-boundary repair, after-insert landing correction, bare-row
  auto-prefix stripping, and true 3-way merge recovery.

---

## Web UI overhaul — TanStack Router + Virtual + shadcn + multi-device — COMPLETE

### Routing (file-based, path-based)

- TanStack Router with `@tanstack/router-plugin/vite` + `autoCodeSplitting`
- `src/routes/__root.tsx` — `SidebarProvider` + `SessionRail` + `SidebarInset` wrapping `<Outlet />` + `MobileNav`
- `src/routes/index.tsx` — `/` redirects to last/new session
- `src/routes/session.$sessionId.tsx` — `/session/$sessionId` chat view
- `src/routes/settings.tsx` — `/settings` full-page theme picker with back button
- `src/routeTree.gen.ts` — auto-generated by Vite plugin
- `src/main.tsx` — creates router from generated route tree, wraps in `ThemeProvider`
- `useSessionNavigation()` in `__root.tsx` syncs store sessionId → router

### Virtual chat surface

- `src/components/virtual-chat-surface.tsx` — TanStack Virtual with `followOnAppend`, stable keys via `getItemKey`, `measureElement` for dynamic heights
- Loading state: shows spinner when session switched but no messages yet
- Scrolls to end via `requestAnimationFrame` when messages go from 0 → N (history loaded)
- `min-h-0` on scroll container + `SidebarProvider` changed to `h-svh overflow-hidden` to constrain flex layout

### Fake header + footer

- `src/components/fake-header.tsx` — borderless bar with sidebar toggle, session title, activity sheet button, settings link. Constrained to `max-w-3xl mx-auto`
- `src/components/status-footer.tsx` — borderless status bar beneath input area. Constrained to `max-w-3xl mx-auto`
- Both have `shrink-0` so they don't compress the chat surface

### shadcn migration

- Left sidebar: shadcn `Sidebar` with `variant="floating"` / `variant="inset"`, `collapsible="icon"` / `"offcanvas"`
- Right rail: shadcn `Sheet` (slide-over from right), opened via Activity button in header
- `MobileNav` inside `SidebarProvider`, wired to `setOpenMobile` for session drawer
- `InputArea` reads `isMobile` from `useSidebar()` instead of prop
- `AskUserCard` rendered inline on mobile via `MobileAskUser` component
- All components use shadcn `Button`, `Input`, `Dialog`, `Sheet`, `Sidebar` family
- Added `icon-sm` button size variant for sidebar triggers
- Installed `radix-ui` for `Slot` support
- All component files renamed to kebab-case (22 files)

### Theme system

- `src/lib/theme.tsx` — loads `themes.json`, sets `data-theme` on `<html>`, `mode` (light/dark) from luminance
- CSS already has 21 `[data-theme="..."]` blocks with hand-tuned HSL palettes including sidebar vars
- Persisted to `localStorage` (`mew-theme-id`), defaults to `catppuccin-mocha`
- Settings page at `/settings` with search, theme grid grouped by category, color swatch previews

### Multi-device sync

- `UserMessage` wire type: daemon broadcasts user's prompt text to all attached clients
- Sending client deduplicates by matching text content in last message
- `attach_session` always sends full `SessionHistory` (removed `was_first` guard)
- All streaming events (`provider`, `tool_start`, `tool_end`, `part_updated`, etc.) broadcast to all clients via `session.broadcast()`

### Session management

- Auto-delete empty sessions: `list()` checks `session.jsonl` size, removes directories with empty/missing files
- Skip active sessions with 0 messages from session list
- `last_message_at` added to `Meta` struct, updated on every `write_message` call
- Session list sorted by `last_message_at` descending (most recent first)
- Auto titles: after first turn, daemon derives title from first user message (60 char truncation), broadcasts `SessionTitleChanged`
- `title_generated` flag on `Session` prevents re-generation
- Future: AI-generated titles (swap `derive_session_title` for provider call, ~20 line change)

### Session resume fix (serde aliases)

- Old on-disk data has PascalCase `Finish` (`"Stop"`, `"ToolUse"`) and `ErrorKind` values
- Added `#[serde(alias = "...")]` to every variant for backward compatibility
- 3 tests in `mew-message` verifying old-format deserialization

### `just restart-daemon` recipe

- Kills bridge + daemon, rebuilds `-p mew-web-bridge -p mew`, cleans socket, restarts
- `just kill-daemon` now also kills the daemon process (not just the bridge on port 9847)

### snake_case wire protocol

- Fixed all PascalCase → snake_case in `session.ts`: `matchToolState`, `wirePartToMessagePart`, `onProviderEvent`, `onPartUpdated`, `onSubagentEnd`
- Fixed in `right-rail.tsx` and `subagent-panel.tsx`
- JSDoc comments in `mew-web-client/src/index.ts` updated
- Rebuilt `@mew/web-client` dist

### Layout fixes

- `SidebarProvider`: `h-svh overflow-hidden` (was `min-h-svh`)
- `SidebarInset`: `min-h-0` added to constrain flex growth
- `VirtualChatSurface`: `min-h-0 flex-1 overflow-y-auto` with `h-svh` parent
- `FakeHeader`, `InputArea`, `StatusFooter`: `shrink-0` to prevent compression
- Sidebar width: 20rem (was 16rem)

### shell_monitor brace fix

- Fixed pre-existing unclosed delimiter in `mew-agent/src/tools.rs` — missing closing brace in `else` block

**Verification:** `tsc --noEmit` clean, `vite build` succeeds, `cargo build -p mew` clean, `cargo test -p mew-message` 26 tests pass.

### Next steps

- Bash command segment classification (split compound commands, rank danger per segment) (edit: done)
- AI-generated session titles (swap text truncation for provider call) (edit: done)
- Additional settings: session management UI, connection settings, model defaults

---

## Snake_case wire protocol — TS client + store + daemon rebuild — COMPLETE

The `mew-web-client` wire types and `mew-protocol`/`mew-message` Rust enums
all use `rename_all = "snake_case"`, but the TS store (`session.ts`) was still
matching PascalCase wire strings (`"PartStart"`, `"Text"`, `"ToolCall"`,
`"Pending"`, `"Completed"`, `"Cancelled"`, `"Failed"`, etc.). Every switch case
fell through to default, so nothing rendered. Also the running daemon binary
was stale (pre-snake_case).

### What was done

- Fixed all PascalCase → snake_case in `session.ts`: `matchToolState`,
  `wirePartToMessagePart`, `onProviderEvent` (all cases), `onPartUpdated`,
  `onSubagentEnd`.
- Fixed `RightRail.tsx`: `sub.outcome?.type === "failed"`.
- Fixed `SubagentPanel.tsx`: same outcome type fix.
- Fixed JSDoc comments in `mew-web-client/src/index.ts`: all backtick-quoted
  references (`NewSession` → `new_session`, `ListModels` → `list_models`,
  etc.).
- Rebuilt `mew-daemon` and restarted the daemon so the wire sends snake_case.
- Rebuilt `@mew/web-client` dist so TS types match.

## Settings modal + named theme system — COMPLETE

Replaced the old light/dark/system `ThemeToggle` with a proper settings modal
backed by the 21 named themes in `themes.json`.

### Theme system (`lib/theme.tsx`)

- Rewritten to load `themes.json` and expose `THEMES`, `useTheme()`.
- Switching sets `data-theme="<id>"` on `<html>` — the CSS already has all 21
  `[data-theme="..."]` blocks with hand-tuned HSL palettes (sidebar vars too).
- `mode` (light/dark) derived from background luminance for shiki highlighting.
- Persisted to `localStorage` (`mew-theme-id`), defaults to `catppuccin-mocha`.

### SettingsModal component

- Modal with search, theme grid grouped by category, color swatch previews.
- Checkmark on active theme. Escape to close. Click-outside to close.

### Access points wired

- `StatusFooter`: gear icon opens settings (replaced `ThemeToggle`).
- `SessionRail`: gear icon in header (replaced `ThemeToggle`).
- `MobileNav`: "More" tab opens settings (was toggling `.dark` class).
- `App.tsx`: `settingsOpen` state, renders `<SettingsModal>`.
- Deleted `ThemeToggle.tsx` (no longer used).
- `CodeBlock.tsx`: `resolved` → `mode` (renamed in new theme context).
- `shiki.ts`: `ResolvedTheme` → `ResolvedMode` type rename.
- `tsconfig.json`: added `resolveJsonModule` for `themes.json` import.

**Verification:** `tsc --noEmit` clean. `@mew/web-client` rebuilt.

---

## `just dev-web-watch` — full Rust + Vite watch mode

Added a new recipe that auto-reloads the Rust side as well:

- `just dev-web-watch` starts the Vite dev server in the background.
- Uses `cargo watch` to rebuild/restart both the daemon and the bridge whenever
  Rust sources under `crates/` change.
- Tears down the stale daemon socket/process before each restart so the fresh
  bridge spawns a fresh daemon.
- `--open` launches `http://127.0.0.1:5173/`.
- New helper script: `scripts/dev-web-watch.sh`.

Requires `cargo-watch` (already installed in this environment).

Also fixed an unbound-array bug in the `dev-web` recipe when called with no
flags (e.g. `just dev-web`).

## Fixed message render failures

Several `MessageItem` children could throw at runtime:

- `ToolCallCard` `StateIcon` had an inexhaustive switch; an unexpected state
  value made it return `undefined` from a component, which React treats as an
  error.
- `ToolCallCard` now falls back to `pending` if no state is present.
- `ReasoningBlock` now coerces incoming text to a string so an undefined/null
  reasoning payload doesn't crash.

## Fixed sessions/models disappearing on session switch

The store's `reset()` action was clearing global caches (`availableModels`,
`availableSessions`, `sessionTitles`) and the current model whenever a new
session was started or attached. Since the UI reads those lists directly,
clicking a session made the left rail and model pickers go blank.

Fix: `reset()` now only clears session-specific state (messages, tool state,
permissions, subagents, todos, token totals). Global model/session caches and
`currentModel/currentProvider/currentThinkingVariant` are preserved and updated
by their normal wire events.

## `just dev-web` now uses Vite dev server

Changed `dev-web` to run the Vite dev server with hot-module reload instead of
forcing a production build each time:

- Removed the `build-web` dependency from `dev-web`.
- Starts the `mew-web-bridge` in the background.
- Waits for the bridge TCP listener to bind.
- Runs `pnpm --filter mew-web-ui dev`.
- Cleans up the bridge process on exit.
- `--open` now launches `http://127.0.0.1:5173/` (Vite) rather than the bridge.

`just --list` parses cleanly.

---

## Web UI — Claude Code-style input composer

Redesigned the composer to match the minimal Claude Code input:

- Single rounded bar (`rounded-xl`) with a subtle `bg-muted/40` fill.
- Attach, textarea, and send/stop are all inside one bar.
- Placeholder reduced to "Ask mew anything…" with `/` and `@` palettes that
  still open when typing.
- A compact row beneath the bar holds:
  - Attachment chips on the left.
  - Small `ModelPill` and `PersonaPill` on the right.
- Re-added `PersonaPill.tsx` in a smaller, more compact form.

---

## Web UI — removed top bar

Pushed the UI closer to the Claude Code-web minimal density:

- Deleted `TopBar.tsx` (and the now-unused `ModelPicker.tsx` and
  `TitleStrip.tsx`).
- Moved the session label into the left-rail header under the workspace name.
- Moved the theme toggle into the status footer and the left-rail header.
- Top safe-area inset is now handled by the root container.
- Build passes (`npm run build`).

---

## Web UI design differentiators — COMPLETE

Implemented the full set of `mew-web-ui/DESIGN.md` differentiators, plus the
mobile treatment already in progress. Build passes (`npm run build`).

### Layout

- Three-pane desktop layout: persistent left rail, chat column, collapsible
  right rail.
- Mobile falls back to slide-over session drawer + stacked subagent/todo/ask
  panels above the composer.
- `TopBar`: connection, session id, model picker, tokens/cost, theme toggle.

### Persistent left rail (`SessionRail.tsx`)

- Workspace header with "mew / Workspace" label.
- "New session" primary button + "Continue latest" secondary button.
- Session rows: title from `sessionTitles` (or model/id fallback), state badge,
  model shortcut, relative age, active-session accent bar.

### Signature status footer (`StatusFooter.tsx`)

- Hidden until connected; fades in.
- Left: connection dot + state + `ModelPill` picker.
- Middle: tokens in/out, cost, active subagent count, pending permission count.
- Right: active persona badge.
- Tabular-nums for stable metrics.

### Input composer (`InputArea.tsx`)

- Above textarea: `ModelPill` and `PersonaPill`.
- `/` opens a command palette (clear, compact, help).
- `@` opens a persona palette.
- Attach button + attachment chip list.
- Focus hints: "Cmd/Ctrl + Enter to send", "Esc to cancel".
- `Cmd/Ctrl+K` global shortcut focuses composer.

### Tool-call surface language (`ToolCallCard.tsx`)

- State icon + label + elapsed timer.
- Sensitivity badge: ReadOnly / Mutating / Dangerous.
- Red border for errors; inline error reason.
- Copy button on output with copied flash.
- Run-args diff view for `edit` and `write` tools.

### Reasoning timeline (`ReasoningBlock.tsx`)

- Collapsed chip: "Reasoning · N steps".
- Expanded vertical timeline with numbered steps.
- Streaming reasoning shows a live typing indicator.

### Right rail (`RightRail.tsx`)

- Tabs: Todos, Subagents, Questions with active-count badges.
- Todo rows with dependency expansion.
- Subagent rows with status dot, tool name, progress `↳`, failure reason.
- Questions reuse `AskUserForm` from `AskUserCard.tsx`.

### Empty states + polish

- `ChatSurface` empty state: icon, heading, hint, recent sessions, new session
  button.
- Added `popover` color tokens to `index.css`.
- Wired `persona-switch-requested` event to `currentPersona` store field.
- Added `time` to tool-call parts so duration can be computed.

### Files changed

- `mew-web-ui/src/App.tsx` — three-pane layout, `RightRail`, focus shortcut,
  slash handler.
- `mew-web-ui/src/stores/session.ts` — persona state + event, tool-call time.
- `mew-web-ui/src/index.css` — popover tokens.
- `mew-web-ui/src/components/SessionRail.tsx`, `StatusFooter.tsx`,
  `InputArea.tsx`, `ToolCallCard.tsx`, `ChatSurface.tsx`, `AskUserCard.tsx`
  upgraded.
- `mew-web-ui/src/components/RightRail.tsx`, `ReasoningBlock.tsx`,
  `PersonaPill.tsx` new.
...
## Web UI mobile treatment — COMPLETE

Full mobile-responsive redesign of the web UI. Installable as a PWA,
usable on a phone.

### Responsive layout

- **SessionRail**: on mobile (<768px), renders as a slide-over drawer
  with backdrop instead of a fixed sidebar. Animated slide transition.
  Desktop keeps the fixed sidebar.
- **TopBar**: hides token/cost info and model picker on mobile. Keeps
  connection status + session button visible. Safe-area-inset padding
  for notched phones.
- **ChatSurface**: tighter padding on mobile (px-3 vs px-4), keeps
  max-w-3xl centered on desktop.

### Mobile input

- **Enter to send** on mobile (Shift+Enter for newline). Desktop keeps
  Cmd/Ctrl+Enter.
- Tap targets bumped to 44px (h-11 w-11) on mobile, 40px on desktop.
- Safe-area-inset bottom padding for home indicator.
- Text size 16px on mobile (prevents iOS zoom on focus).

### PWA

- `manifest.json` with standalone display, theme color, app icon
- `apple-touch-icon` + `apple-mobile-web-app-capable` meta tags
- `viewport-fit=cover` for full-screen on notched devices
- `theme-color` meta tag
- Favicon from the existing mew logo SVG

### Mobile bottom navigation

- `MobileNav` component: Chat / Sessions / More tabs
- Sessions tab toggles the slide-over drawer
- More tab cycles the theme
- Hidden on desktop (md:hidden)

### Files changed

- `mew-web-ui/src/App.tsx` — `useIsMobile` hook, responsive layout,
  MobileNav, drawer pattern
- `mew-web-ui/src/components/SessionRail.tsx` — `isMobile` prop,
  mobile width + close button
- `mew-web-ui/src/components/TopBar.tsx` — `isMobile` prop, hide
  tokens/cost/model picker on mobile, safe-area padding
- `mew-web-ui/src/components/InputArea.tsx` — `isMobile` prop,
  Enter-to-send, bigger tap targets, safe-area padding
- `mew-web-ui/src/components/ChatSurface.tsx` — responsive padding
- `mew-web-ui/src/components/MobileNav.tsx` — new component
- `mew-web-ui/index.html` — PWA meta tags, manifest link, viewport-fit
- `mew-web-ui/public/manifest.json` — PWA manifest
- `mew-web-ui/public/favicon.svg` — app icon (from site logo)

**Verification:** `tsc --noEmit` clean, `pnpm build` succeeds,
`cargo build --workspace` clean.

---

## Built-in mew-docs skill — COMPLETE

Added a built-in `mew-docs` skill (analogous to Polytoken's
`polytoken:modifying-polytoken`). When the agent needs to understand
mew-specific behavior, it loads this skill and gets a documentation
sitemap pointing at `https://mew.computer/docs/<slug>`. The agent then
uses `web_fetch` to read the specific page it needs.

### What was done

- New `SKILL.md` at `crates/mew-prompts/resources/skills/mew-docs/` —
  frontmatter + sitemap of 23 doc pages with descriptions
- `mew-skills` gains `builtin_skills()` + `load_skill_from_str()` —
  embedded via `include_str!`, same override-by-name pattern as personas
- `Loader::load()` now appends built-in skills after disk-discovered
  ones (user skills win on name conflict)
- Fixed Astro config: `site: "https://mew.computer"` (was `"mew.sh"`)
- 3 new tests (built-in loaded, has sitemap, included in load)
- Fixed `test_duplicate_name_project_wins_over_global` missing
  `with_test_home()` (pre-existing bug — discovered real home skills)

### Files changed

- `crates/mew-prompts/resources/skills/mew-docs/SKILL.md` — new
- `crates/mew-skills/src/lib.rs` — `builtin_skills()`,
  `load_skill_from_str()`, built-in loading in `Loader::load()`, 3 tests
- `site/astro.config.mjs` — corrected site URL to `mew.computer`

**Verification:** `cargo fmt`, `cargo clippy -- -D warnings`,
`cargo test --all` — all clean. 11 skill tests pass.

---

## TUI theming Phase 3+4 — Built-in themes + CLI — COMPLETE

### Built-in themes

Three themes embedded as JSON via `include_str!`:
- **catppuccin-mocha** (dark) — warm dark palette from the Catppuccin
  Mocha color scheme
- **catppuccin-latte** (light) — light Catppuccin variant
- **tokyo-night** (dark) — blue-tinted dark palette inspired by the
  VS Code Tokyo Night theme

All three load via `Theme::load("catppuccin-mocha")` etc., and appear
in `/theme` listing and `mew theme list`. 4 tests.

### `mew theme` CLI subcommand

- `mew theme list` — lists available themes with `*` marking the active
- `mew theme current` — prints the active theme name
- `mew theme install <path>` — validates a JSON theme file and copies
  it to `~/.config/mew/themes/`

### Files changed

- `crates/mew-tui/resources/themes/catppuccin-mocha.json` — new
- `crates/mew-tui/resources/themes/catppuccin-latte.json` — new
- `crates/mew-tui/resources/themes/tokyo-night.json` — new
- `crates/mew-tui/src/theme.rs` — embedded built-in loading,
  `themes_dir()` public method, 4 tests
- `crates/mew/src/main.rs` — `ThemeCommands` enum + `theme_cmd()` handler

**Verification:** `cargo fmt`, `cargo clippy -- -D warnings`,
`cargo test --all` — all clean. 25 theme tests. CLI verified live:
`mew theme list`, `mew theme current`, `mew theme --help`.

---

## TUI theming Phase 2 — Light theme + config + /theme command — COMPLETE

### Light theme

`Theme::light()` with proper light-mode colors: near-white backgrounds,
near-black text, inverted pill tiers (dark fg, light bg). Mirrors the
web UI's `:root` light tokens. 2 tests (luminance checks, tier inversion).

### Theme discovery + loading

`Theme::load(name)` searches:
1. Built-in: `"dark"` → `Theme::dark()`, `"light"` → `Theme::light()`
2. `<cwd>/.mew/themes/<name>.json` (project-local)
3. `~/.config/mew/themes/<name>.json` (user-installed)
Falls back to `dark()` if not found.

`Theme::list_available()` returns built-ins + any JSON files in the
search paths. 4 tests for load/list.

### Config wiring

- `config.toml`: `[tui] theme = "dark"` (or "light", or a theme name)
- `state.toml`: `theme = "..."` (runtime override, set by `/theme`)
- Priority: state.toml > config.toml > "dark" default
- Both TUI construction paths (local + daemon client) load the theme
  at startup

### `/theme` slash command

- `/theme` — lists available themes with `*` marking the active one
- `/theme light` — switches to light theme immediately (no restart)
- `/theme dark` — switches back to dark
- `/theme catppuccin` — loads a user-installed theme file
- Theme choice persisted to `state.toml` on switch
- Registered in `builtin_slash_commands()` for autocomplete

### Files changed

- `crates/mew-tui/src/theme.rs` — `Theme::light()`, `Theme::load()`,
  `list_available()`, `config_themes_dir()`, 7 new tests
- `crates/mew-config/src/lib.rs` — `TuiConfig`, `State.theme` field
- `crates/mew-tui/src/app.rs` — `SlashResult::SetTheme`, `/theme`
  parsing, `/theme` builtin command
- `crates/mew/src/main.rs` — theme loading at both App construction
  paths, `SetTheme` handling at 3 slash-result match sites

**Verification:** `cargo fmt --check`, `cargo clippy --workspace
-D warnings`, `cargo test --all` — all clean. 22 theme tests pass.

---

## TUI theming Phase 1 — Theme struct + token replacement — COMPLETE

Implemented the first phase of the theming system per `THEMING_PLAN.md`.

### Theme struct + JSON loading

New `Theme` struct in `mew-tui/src/theme.rs` with:
- 30 core shadcn v4 tokens (`background`, `foreground`, `card`, `popover`,
  `sidebar_*` family, `chart_*`, etc.)
- 3 mew surface extensions (`status_bg`, `tool_bg`, `divider`)
- 6 three-tier semantic color scales (`red`, `green`, `yellow`, `blue`,
  `purple`, `cyan` — each with `fg`/`med`/`bg`)
- `Theme::dark()` returns exact current hardcoded values
- `Theme::from_json()` / `from_json_str()` loads theme files, merging
  over `dark()` defaults for partial themes
- `ThemeMode` enum (Dark/Light) with serde
- 15 tests (7 existing accent + 8 new theme loading)

### Hardcoded colors replaced

All ~135 color references across the TUI now read from `app.theme.tokens`:
- Removed `STATUS_BG`, `SIDEBAR_BG`, `TOOL_BG`, `DIVIDER` constants from
  `ui/mod.rs`
- `App` gains `theme: Theme` field (defaults to `Theme::dark()`)
- `draw_divider`, `draw_permission_modal`, `draw_user_question*`,
  `draw_picker`, `draw_persona_confirm_modal` now take `&ThemeTokens`
- `gap_segment` and `build_segments` take `tokens` parameter
- Tests updated to construct `App::new()` for theme access

### Files changed

- `crates/mew-tui/src/theme.rs` — Theme struct, ThemeTokens, ColorScale,
  JSON serde, ThemeError, 8 new tests
- `crates/mew-tui/src/app.rs` — `theme` field + `active_persona_color`
- `crates/mew-tui/Cargo.toml` — added `serde` dependency
- `crates/mew-tui/src/ui/mod.rs` — removed constants, pass tokens
- `crates/mew-tui/src/ui/status.rs` — all colors from tokens
- `crates/mew-tui/src/ui/input.rs` — all colors from tokens
- `crates/mew-tui/src/ui/overlays.rs` — all colors from tokens
- `crates/mew-tui/src/ui/chat.rs` — all colors from tokens
- `crates/mew-tui/src/ui/sidebar.rs` — all colors from tokens
- `crates/mew-tui/src/ui/welcome.rs` — all colors from tokens

**Verification:** `cargo fmt --check` clean, `cargo clippy --workspace
-- -D warnings` clean, `cargo test --all` — all tests pass.
